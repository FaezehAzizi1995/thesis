...
Original file is located at
    https://colab.research.google.com/drive/113QufbHDWjiGIjvHj5GxVLDbwD5WVP4m
...
import umap
umap_embeddings = umap.UMAP(n_neighbors=4, n_components=2, metric='cosine').fit_transform(embeddings)
#کاهش بعد به 5---همسایه های محلی 15
...
fcm = FCM(n_clusters=5) # we use two cluster as an example
fcm.fit(X.to_numpy())
...
import pandas as pd
# Prepare data
umap_data = umap.UMAP(n_neighbors=2, n_components=2, min_dist=0.0, metric='cosine').fit_transform(embeddings) #کاهش بعد به 2
...
centers = fcm.centers
labels = fcm.predict(Z.to_numpy())

from sklearn.metrics import davies_bouldin_score
db_index = davies_bouldin_score(embedd, labels)
print(db_index)
...
for i in range(3,10):
    cluster= FCM(n_clusters=i)
   # fcm = FCM(n_clusters=8) # we use two cluster as an example
    cluster.fit(Z.to_numpy())
   # cluster = hdbscan.HDBSCAN(min_cluster_size=i, metric='euclidean', cluster_selection_method='eom').fit(umap_embeddings)
   # umap_data = umap.UMAP(n_neighbors=2, n_components=2, min_dist=0.0, metric='cosine').fit_transform(embedding)
    result = pd.DataFrame(embedd, columns=['x', 'y'])
    result['labels'] = cluster.predict(Z.to_numpy())
    db_index = davies_bouldin_score(embedd, cluster.predict(Z.to_numpy()))
    print(db_index)
    results.update({i: db_index})
...
for i in range(3,10):
  ...
  #  umap_data = umap.UMAP(n_neighbors=2, n_components=2, min_dist=0.0, metric='cosine').fit_transform(embeddings)
    ...
    result['labels'] = cluster.predict(Z.to_numpy())
    #db_index = davies_bouldin_score(umap_embeddings, cluster.labels_)
    silhouette = silhouette_score(embedd, cluster.predict(Z.to_numpy()))
    print(silhouette)
    results.update({i: silhouette})
...
for i in range(3,10):
   ...
   # umap_data = umap.UMAP(n_neighbors=2, n_components=2, min_dist=0.0, metric='cosine').fit_transform(embeddings)
    ...
    #db_index = davies_bouldin_score(umap_embeddings, cluster.labels_)
    #silhouette = silhouette_score(umap_embeddings, cluster.labels_)
    CH = metrics.calinski_harabasz_score(embedd, cluster.predict(Z.to_numpy()))
    print(CH)
    results.update({i: CH})
